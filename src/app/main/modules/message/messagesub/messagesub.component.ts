...
@Component({
  selector: 'app-messagesub',
  templateUrl: './messagesub.component.html',
  styleUrls: ['./messagesub.component.css']
})
export class MessagesubComponent implements OnInit,OnDestroy,AfterViewInit{
  ...
  async SendMessageEventListen(){
    this.sendMessageUnsubscribe$ = this._SharedService.listen('sendMessage').subscribe(async (res:any) => {
      await this.singleChatMessage(res);
      if(res[0].senderUser_details.username == this._AuthenticationService.currentUserValue.username){
        const receiverIndex = await this.ChatMemberList.findIndex((ele:any)=>{return ele.user_details._id == res[0].receiverUser_details._id});
        if(receiverIndex === -1){
          const responsemessage = {
            messages:{message:res[0].message,self_message:true,created_date:res[0].created_date},
            user_details: res[0].receiverUser_details,
            _id:res[0].chatroom_id
          }
          await this.ChatMemberList.push(responsemessage)
        }else{
          if(this.ChatMemberList[receiverIndex].messages == undefined){
            this.ChatMemberList[receiverIndex]['messages']={message:res[0].message,self_message:true,created_date:res[0].created_date}
          }else{
            this.ChatMemberList[receiverIndex].messages.message = res[0].message
            this.ChatMemberList[receiverIndex].messages.self_message = true
            this.ChatMemberList[receiverIndex].messages.created_date = res[0].created_date
          }
        }
      }
      else{
        const senderIndex = await this.ChatMemberList.findIndex((ele:any)=>{ return ele.user_details._id == res[0].senderUser_details._id});
        if(senderIndex === -1){
          const responsemessage = {
            messages:{message:res[0].message,self_message:false,created_date:res[0].created_date},
            user_details: res[0].senderUser_details,
            _id:res[0].chatroom_id
          }
          this.ChatMemberList.push(responsemessage)
        }
        else{
          if(this.ChatMemberList[senderIndex].messages == undefined){
            this.ChatMemberList[senderIndex]['messages']={message:res[0].message,self_message:false,created_date:res[0].created_date}
          }else{
            this.ChatMemberList[senderIndex].messages.message = res[0].message
            this.ChatMemberList[senderIndex].messages.self_message = false
            this.ChatMemberList[senderIndex].messages.created_date = res[0].created_date
          }
        }
      }
  ...
  singleChatMessage(res:any){
    if(res[0].senderUser_details.username == this._AuthenticationService.currentUserValue.username){
      const unreadMessagesData = {
        receiverId:res[0].receiver_id,
        messageId:res[0]._id
      }
      this._SharedService.emit('unreadMessages', unreadMessagesData);
      if(this.ActiveUserChatDetail && this.ActiveUserChatDetail.user_details._id == res[0].receiverUser_details._id){
        const setBottomMsg = {
          chatroom_id:res[0].chatroom_id,
          created_date:res[0].created_date,
          message:res[0].message,
          receiver_id:res[0].receiver_id,
          self_message:true,
          sender_id:res[0].sender_id,
          user_details:res[0].senderUser_details,
          images:res[0].images,
          documents:res[0].documents,
          productDetails:res[0].productDetails,
          replyCount:res[0].replyCount,
          isEdited:res[0].isEdited,
          quotesMessage_details:res[0].quotesMessage_details,
          repostedMessageId:res[0].repostedMessageId,
          isReposted:res[0].isReposted,
          _id:res[0]._id
        }
        this.singleChatMessageList.push(setBottomMsg)
        setTimeout(() => {
          const element:any = document.getElementById(`message${this.singleChatMessageList.length-1}`);
          if(element != null) element.lastChild.scrollIntoView(false);
        }, 0);
      }
      
    }else{
      if(this.ActiveUserChatDetail && this.ActiveUserChatDetail.user_details._id == res[0].senderUser_details._id){
        const setBottomMsg = {
          chatroom_id:res[0].chatroom_id,
          created_date:res[0].created_date,
          message:res[0].message,
        ...
        this.singleChatMessageList.push(setBottomMsg)
        this.readUnreadMessagesEmitEvent(res[0].sender_id)
        setTimeout(() => {
          const element:any = document.getElementById(`message${this.singleChatMessageList.length-1}`);
          if(element != null) element.lastChild.scrollIntoView(false);
        }, 0);
        let audio = new Audio();
        audio.src = "assets/audio/Hustleup message tone.mp3";
        audio.load();
        audio.play();
      }
    }
  ...
  GetAllChatMembers(){
    ...
    this._MessagechatService.getAllChatMembers(data).subscribe((res: any) => {
      if (res.status === 200) {
        ...
        if(this.activeUserId){
          const selectedMemeber = this.ChatMemberList.filter((item: any) => item._id == this.activeUserId);
          if(selectedMemeber.length > 0) {
            this.userchatdetails(selectedMemeber[0]);
          }
        }
      }
    }, (err => {
  ...
  replyListApi(id:any,pageno:any){
    ...
    this._MessagechatService.getChatMessagesListReplysList(data).subscribe(async(res:any)=>{
      console.log(res,"resAllrepliesList")
      if(res.status == 200){
  ...
  DeleteMessage(item:any,index:any){
    console.log(item,"DeleteMessageitem")
  ...
  closesidebar(){
    this.replySelectedItem = undefined;
